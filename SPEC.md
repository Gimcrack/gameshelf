...
T22|.|advanced filtering: pull IGDB themes/keywords/game_modes onto `games` (extend IgdbGameAttributes + IgdbClient field lists, populate at match/create time), extend GET /api/library filter vocab (theme/keyword/game_mode params), FE filter UI. User freeform tags (T7) unaffected|V4,V30,I.api
T23|.|manual collections: `collections.type` enum(filter\|manual), `collection_games` pivot, POST/DELETE /api/collections/:id/games, FE add-to-collection / remove-from-collection affordance|V29,I.api
T24|.|game detail view: GET /api/library/:game_id show route, FE /games/:game_id page — cover/genres/platforms (+ deck_status badge per platform, T26)/time_to_beat/themes/keywords/game_modes/esrb_rating/multiplayer+coop+local badges (T27) + editable status/tags/rating/notes via existing PUT meta|I.api
