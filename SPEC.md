...
- api: `POST /api/library/:game_id/rematch` {igdb_id} → repoints entry's canonical game (V34): reuses existing `games` row if igdb_id already matched elsewhere (V7), else creates one; orphaned unowned provisional row deleted. Works on provisional AND already-matched games — corrects a wrong auto-match too, ⊥ provisional-only. 404 if game_id ∉ caller library. Query-override search for candidates reuses existing `GET /api/discover/search` (I.igdb, T15) — no new IGDB call path. Added T29.
...
- api: `POST /api/library/sync-igdb` → 202, dispatch bulk IGDB sync job (V38) for caller's whole library: matches provisional games (∀ connection) + refreshes already-matched games' canonical attrs. Throttled per-user ≥5 min gap (mirrors connection "sync now", §C) → 429 if too soon. No live progress — v1, caller reloads later. Added T31.
...
V34: manual "fix match" (T29) reuses V7 canonical-row dedupe exactly (mirrors GameMatcher::canonicalize) — repoint `owned_games.game_id` to the chosen canonical row (existing or newly created), delete the orphaned unowned provisional row. Available regardless of current match state (provisional or already-matched) — lets a user correct a wrong auto-match, ⊥ just fill in misses.
...
T38|.|library union + Library Status filter: GET /api/library unions wishlist_items + meta-orphans w/ owned rows, `library_status` per entry (precedence V42), `library_status[]` param; stats/backlog/system collections owned+free only; FE sidebar FacetFilter (static owned\|free\|wishlist\|none) + wishlist badge on GameCard|V42,V21,V28,I.api
T39|.|interactive star rating on library view: GameCard 5 inline star emojis (★ filled/☆ hollow from meta.rating), click star n → PUT /api/library/:game_id/meta {rating: n}, click current rating → clear (null)|V6,I.api
T40|.|personal rating facet: `rating[]` param (1\|2\|3\|4\|5\|none — none = rating IS NULL); FE sidebar FacetFilter, static options w/ star labels + "Unrated"|I.api,V28
